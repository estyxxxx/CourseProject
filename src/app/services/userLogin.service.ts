...
export class UserLoginService {
  ...
  checkLogin(name: string, password: string): Observable<User> {
    ...
    const apiUrl =`https://meetings-app-server-svds.onrender.com/api/User?name=${name}&password=${password}`;
  ...
  addUser(username: string, address: string, email: string, password: string) {
    const url = 'https://meetings-app-server-svds.onrender.com/api/User';
    const userData = {
      name: username,
      address: address,
      email: email,
